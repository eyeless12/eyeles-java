...
import edu.java.repository.IChatRepository;
import edu.java.repository.ILinkRepository;
...
public class LinkUpdaterService implements ILinkUpdaterService {
                        ...
                        .ifPresent((githubResponse -> {
                            if (githubResponse.lastActivityDate().isAfter(link.lastCheckTime())) {
                                trackerBotClient.sendUpdate(
                                    ...
                                    chatRepository.findById(link.id()).stream().map(Chat::id).toList()
                                );
                            }
                        }));
                },
                url -> {
                    GithubRepositoryRequestDto request = toGithubRepository(url);
    ...
    private GithubRepositoryRequestDto toGithubRepository(URL url) {
        String[] parts = url.getPath().split("/");
        return new GithubRepositoryRequestDto(parts[1], parts[2]);
    }
